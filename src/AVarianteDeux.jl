...
function AVarianteDeux(G,vD,vA)
    #Variable
    graphe =Graphe(G)
    ...
    return false
        ...
        while !(isempty(pq.elements))
            ...
            if u == vA
                fin = time() 
                println("l'algortihme wA* Variante 2 :")
            ...
            if !(v in permanent) && !(haskey(graphe[1],v)) && v[1]>0 && v[2]>0 && v[2]<=weigt && v[1]<=height
                ...
                if haskey(distance,v)
                    if new_distance < distance[v]
                        ...
                        parent[v] = u
                        h=Heureutisque(vA,v)
                        update(pq,v,Float64((w*h + new_distance)))
                    end 
                else
                    distance[v] = new_distance
                    parent[v] = u
                    h=Heureutisque(vA,v)  
                    evaluer=evaluer +1
                        ajouterdebut(pq,v,Float64((w*h + new_distance)) ) 
...
AVarianteDeux("dat/theglaive.map",(189,40),(226,437))